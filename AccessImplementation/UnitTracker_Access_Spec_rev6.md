...
SELECT
    ...
    up.Amount,
    up.UnitsPurchased
FROM (UnitPurchase AS up
INNER JOIN AccountValue AS av ON av.ID = up.DateID)
INNER JOIN Funds AS f ON f.ID = up.FundID
...
Simple dialog to add or edit a standalone AccountValue record without an associated purchase. Accessed via Advanced > Add Account Value (Alt+V). In add mode, check if there already is a value for the date. If so, place that value in the Value box and disable entry.
...
Called automatically on frmMain Open and by Advanced > Backup Now (Alt+B). Copies the current .accdb to a /backup subfolder with a timestamp, mirroring the Python `backupDB()` method. In addition to performing the backup, the module checks how many copies of the backup exist and warns the user if too many have accumulated.
...
Public Sub BackupDB()
    ...
    If Err.Number <> 0 Then 
        MsgBox "Backup failed: " & Err.Description, vbExclamation, "Backup"
        On Error GoTo 0
        Exit Sub
    End If
    On Error GoTo 0

    ' Check existing backups AFTER creating the new one
    Dim backupCount As Long
    Dim totalSpace As Currency
    Dim fileName As String
    
    fileName = Dir(backupDir & "\" & baseName & "_*.accdb")
    Do While fileName <> ""
        backupCount = backupCount + 1
        totalSpace = totalSpace + FileLen(backupDir & "\" & fileName)
        fileName = Dir
    Loop
    
    ' Warn if more than 10 backups exist
    If backupCount > 10 Then
        Dim spaceMB As Double
        spaceMB = totalSpace / (1024 * 1024)
        MsgBox "You have " & backupCount & " backups using " & Format(spaceMB, "0.0") & " megabytes of disk space. You might want to consider deleting some of the older ones.", vbExclamation, "Backup Warning"
    End If
End Sub
```
...
Public Function SafeStr(s As String) As String
    SafeStr = Replace(Replace(Trim(s), "'", "''"), """", """""")
End Function
```
